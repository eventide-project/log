...
The `info` level is used to log the completion of the principle operation of a class or utility, like a callable class or object's actuator.
...
The `debug` level is used to log the completion of a secondary operation of a class or utility, or for recording other details.
...
The `error` level is used to log an error message to record an error state immediately before raising the error.
...
The `fatal` level is used only when a process is terminating due to an error. It should only be used by a top-level error handler just before the process terminates.
